...
// const timeZoneOptions = document.querySelectorAll("option");
// LocalTimezone.value = timezone;
// LocalTimezone.innerText = timezone;
// timeZoneOptions.forEach(e => {
...
var predictionIDs = [], users;

releaseDate.setAttribute("min", today.toISOString().split("T")[0]);
releaseDate.value = today.toISOString().split("T")[0];
...
function uploadData(){
    if(predictionVisibility.value == "Public") password.value = null;
    var today = new Date();
    ...
    database.ref(`/users/${auth.currentUser.uid}/${predictionID.value}/`).set({
        "prediction-data":{
            prediction: prediction.value
        },
        private:{
            password: password.value,
            "upload-date": new Date(uploadDateModified).getTime()
        },
        public:{
            "release-timestamp": new Date(releaseDateModified).getTime(),
            tags: publicTags.value
        }

    }).then(e => {
        console.log("Prediction Uploaded.")
        console.log(e, new Date(uploadDateModified).getTime(), uploadDateModified);
